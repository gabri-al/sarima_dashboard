...
layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col([
            html.Div([
                dcc.Slider(50, 95, 5, value=None, marks=None, tooltip={"placement": "bottom", "always_visible": True}, id='train-slider', persistence=True, persistence_type='session')
...
def train_test_split(_trainp, _memo):
    if _trainp:
        _data = pd.DataFrame(_memo['original dataset'])
        if 'transformations_' in _memo.keys(): # Reapply just the log transformation if needed
        ...
        if 'train dataset' in _memo.keys(): # Store info
            _memo.pop('train dataset')
        _memo['train dataset'] = _datatrain.to_dict('records')
        if 'test dataset' in _memo.keys():
            _memo.pop('test dataset')
        _memo['test dataset'] = _datatest.to_dict('records')
        _memo['train_percentage'] = int(_trainp)
...
def dropdown_opt_to(_from, _memo):
    if not _from:
        _from = 0
    if 'train dataset' in _memo.keys():
        _data = pd.DataFrame(_memo['train dataset'])
...
def comb_calc(p_from,p_to,d_from,d_to,q_from,q_to,sp_from,sp_to,sd_from,sd_to,sq_from,sq_to,sm_from,sm_to,_memo):
    ...
    if 'grid_search_params' in _memo.keys():
        _memo.pop('grid_search_params')
    _memo['grid_search_params'] = _combs

    return len(_combs), _memo
...
def grid_search_results(_nclicks, _memo):
    if int(_nclicks) > 0:
        if 'grid_search_params' in _memo.keys():
            if 'train dataset' in _memo.keys():
                _data = pd.DataFrame(_memo['train dataset'])
                _gs_res = sarima_grid_search(_data, _memo['grid_search_params'])
    ...
    if 'grid_search_results' in _memo.keys():
        ...
        return [html.Hr([], className = 'hr-footer'), title_, tbl_], _memo
    else:
        return None, _memo